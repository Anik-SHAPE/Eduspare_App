...
import { createNote, deleteNote, getAllContentByClassRoom, getAllNote, getContent, getEvent, getNote, replaceClassTopic, updateContent, updateEvent, updateNote } from '../helper/teacherapicall';
...
export default function MiniDrawer(props) {
  ...
  const AllcontentLoad = (userId, token,classRoom) => {
    getAllContentByClassRoom(userId, token, classRoom).then(data => {
        if(data.error){
  ...
  const SuccessReplace = () => {
    if(replacesuccess || eventsuccess) {
      return <Redirect to="/institute/admin/calender" />;
    }
  }
        ...
        <div className="hidedesktop fixed-top d-block w-100" style={{height: 70, backgroundColor: "#fafafa"}}>
          <Grid container spacing="1" className="mb-1">
            <Grid item xs={2} sm={6} md={6}>
              <Link to={`/teacher/${pannelId}/content`} style={{textDecoration:"none"}}>
