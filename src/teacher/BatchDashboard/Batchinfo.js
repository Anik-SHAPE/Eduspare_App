...
import {getBatch,removePlannerFromBatch,updateBatch,deleteBatch,getAllBatch,getPlanner,getAllPlanner,createBatch,assignPlannerToBatch} from "../../Institute/helper/teacherapicall";
import {getAllBatchByTeacher, getTeacher, getAllplannerByTeacher} from '../helper/teacherapicall';
  ...
  const userload = () => {
    getUser(user._id, token).then(data => {
        if(data.error){
          ...
          getTeacher(user._id, token, user.pannelID).then(d => {
            if(d.error){
              setTeacherLoad({...TeacherLoad, teacherError: d.error})
            }else{
              setTeacherLoad({
                ...TeacherLoad,
                instId: d.instituteId,
              })
              getBatch(user._id, token, d.instituteId, props.match.params.batchId).then(data=>{
                if(data.error){
                  ...
                  if(data.planner!=""){
                  getPlanner(user._id,token, d.instituteId, data.planner).then(data=>{
                    if(data.error){
                      console.log('error in db');
                    }else{
                      setPlanner({
              ...
              getAllplannerByTeacher(user._id,token,user.pannelID).then(data=>{
                if(data.error){
                  console.log('error in db')
                }else{
                     setPlanners(data);
 ...
 const assignPlanner=event=>{
   ...
   assignPlannerToBatch(user._id,token,props.match.params.batchId,plannerId,{planner_sdate}).then(data=>{
        if(data.error){
          ...
          getPlanner(user._id,token,instId,data.planner).then(data=>{
            if(data.error){
              console.log('error in db');
            }else{
              setPlanner({
