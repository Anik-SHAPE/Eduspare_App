...
import {getBatch,updateBatch,deleteBatch,getAllBatch,getPlanner,getAllPlanner,createBatch} from "../../Institute/helper/teacherapicall";
...
import { createBatchPushToTeacher, getAllBatchByTeacher, getTeacher } from '../helper/teacherapicall';
...
const Batch = (props) => {
  ...
  const ManageBatch = () => {
    ...
    const createThisBatch=event=>{
      ...
      createBatchPushToTeacher(user._id, token, instId, user.pannelID, {batchname, start_time, end_time, weekdays}).then(data=>{
        if(data.error){
     ...
     const getAllBatchByClassRoom=()=>{
      getAllBatchByTeacher(user._id,token,user.pannelID).then(data=>{
        if(data.error){
          console.log('error in db');
        }else{
          setBatchs(data);
     ...
     const getThisBatch=batchId=>{
      getBatch(user._id, token, instId, batchId).then(data=>{
        if(data.error){
          console.log('error in db');
        }else{
          setBatch({
  ...
  const updateThisBatch=event=>{
    event.preventDefault();
      updateBatch(user._id,token, instId,itemId, {batchname,start_time,end_time,weekdays}).then(data=>{
        if(data.error){
          console.log('error in db');
        }else{
          setBatch({
    ...
    const deleteThisBatch=batchId=>{
        deleteBatch(user._id,token,instId,batchId).then(data=>{
          if(data.error){
            console.log("error in db");
          }else{
            getAllBatchByClassRoom();
         ...
         return(
          ...
          <Card className="shadow-sm d-block w-100 h-100" style={{ backgroundColor: "#eceff1"}}>
            <Grid container spacing="0">
              ...
              <Grid item xs={9} sm={10} md={11}>
              <Link to={`/teacher/batch/${batch._id}`} style={{textDecoration: "none"}}>
