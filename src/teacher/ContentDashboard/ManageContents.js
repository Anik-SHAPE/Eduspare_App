...
import { deleteContent ,getContent,updateContent, getAllNote, deleteNote, getNote, updateNote, createNote, getAllContentByClassRoom } from '../../Institute/helper/teacherapicall';
...
import { createContentPushTeacher, getAllContentByTeacher, getTeacher } from '../helper/teacherapicall';
...
const Contents = (props) => {
  ...
  const GetContents = ()  => {
    ...
    const contentLoad = () => {
      getAllContentByTeacher(user._id, token, user.pannelID).then(datas => {
          if(datas.error){
   ...
   const createThisNote=event=>{
    ...
    const deleteThisContent = ContentId => {
      deleteContent(user._id, token, instId, ContentId).then(data => {
        if (data.error) {
          console.log(data.error);
        } else {
      ...
      > 
        <center>
          ...
          <center><Button style={{outline: "none"}} onClick={()=>{
               ...
               GetContentLoad(user._id, token, instId, ContentId);
               setOpen(true);
    ...
    const onSubmit =  event => {
      ...
      createContentPushTeacher(user._id, token, user.pannelID, { topicName, topicDescription, videoLinkOne, videoLinkOneTitle, videoLinkTwo, videoLinkTwoTitle, videoLinkThree, videoLinkThreeTitle, audioLink, audioTitle, subject})
      .then(data => {
        if(data.error){
      ...
      updateContent(user._id, token, instId, ContentId, { topicName, topicDescription, videoLinkOne, videoLinkOneTitle, videoLinkTwo, videoLinkTwoTitle, videoLinkThree, videoLinkThreeTitle, audioLink, audioTitle, subject})
      .then(data => {
        if(data.error){
          setContent({...content, error: data.error})
        }
          ...
          <div>
            <Card className="shadow-sm d-block w-100 h-100 mb-2" style={{ backgroundColor: "#eceff1"}}>
              <Grid  container spacing="0">
                <Grid item xs={9} sm={10} md={11}>
                <Link to={`/teacher/content/${content._id}`} style={{textDecoration: "none"}}><p className="pt-3 pl-3" style={{fontSize: 16, fontWeight: "bold", color: "#000000"}}>{content.subject}
