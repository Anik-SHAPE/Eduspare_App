...
import {getAllPlannerByClassRoom, deletePlanner, getAllPlanner, getPlanner, updatePlanner, createPlanner} from "../../Institute/helper/teacherapicall";
...
import { createPlannerPushTeacher, getAllplannerByTeacher, getTeacher } from '../helper/teacherapicall';
...
const MyPlanner = () => {
  ...
  const LoadAllPlanners=(userId,token,TeacherId)=>{
    getAllplannerByTeacher(userId,token,TeacherId).then(data=>{
      if(data.error){
  ...
  const getThisPlanner=plannerId=>{
    getPlanner(user._id,token,instId,plannerId).then(data=>{
      if(data.error){
   ...
   const updateThisPlanner=event=>{
     ...
     updatePlanner(user._id, token, instId, PlannerId, {planner_name, duration}).then(data=>{
       if(data.error){
  ...
  const createThisPlanner=event=>{
    event.preventDefault();
    createPlannerPushTeacher(user._id,token,user.pannelID,{planner_name,duration}).then(data=>{
      if(data.error){
          ...
          return(
            ...
            <Card className="shadow-sm d-block w-100 h-100" style={{ backgroundColor: "#eceff1"}}>
              <Grid container spacing="0">
                ...
                <Grid item xs={9} sm={10} md={11}>
                <Link to={`/teacher/planner/${planner._id}`} style={{textDecoration: "none"}}>
